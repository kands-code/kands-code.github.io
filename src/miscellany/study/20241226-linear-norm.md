...
例如判断两个向量 \\( v\_1 \\) 和 \\( v\_2 \\) 是否接近，可以尝试计算 \\( \lVert v\_1 - v\_2 \rVert \\)
...
\\( x \cdot y = x^\dagger y = \sum\_{i = 1}^n {\overline{x}\_i y\_i} \\)

其中 \\( x^\dagger \\) 表示对 \\( x \\) 进行共轭转置，
而 \\( \overline{x}\_i \\) 表示将 \\( x \\) 向量的第 \\( i \\) 个元素取共轭

对于实数，\\( x^\dagger = x^T \\)，\\( \overline{x}\_i = x\_i \\)
...
- \\( L \\)-\\( 2 \\) 模：又称为 **欧几里得模**，定义为 \\( \lVert x \rVert\_2 = \sqrt{x \cdot x} = \sqrt{x^\dagger\\,x} \\)
- \\( L \\)-\\( 1 \\) 模：定义为 \\( \lVert x \rVert\_1 = \sum\_{i = 1}^n{|x\_i|} \\)
- \\( L \\)-\\( \infty \\) 模：定义为 \\( \lVert{}x\rVert{}\_{\infty} = \max\_{1 \le i \le n}{|x\_i|} \\)
...
对于 \\( L \\)-\\( 1 \\) 模和 \\( L \\)-\\( 2 \\) 模，
通常有 \\( \lVert x \rVert\_1 \le \sqrt{n} \lVert x \rVert\_2 \\)，
其中 \\( x \in \mathbb{F}^n \\)
...
- 正交矩阵：对于 \\( A \in \mathbb{R}^{n \times n} \\)，如果 \\( A^T\\,A = \mathbf{I} \\)，那么 \\( A \\) 就是正交矩阵

对于正规矩阵 \\( A \in \mathbb{R}^{n \times n} \\)，
我们有 \\( \lVert A^T\\,x \rVert\_2 = \lVert A\\,x \rVert\_2 \\)

\\[
  \begin{aligned}
    \lVert A^T\\,x \rVert\_2^2 &= (A^T\\,x) \cdot (A^T\\,x) = x^T\\,A\\,A^T\\,x \\\\
      &= x^T\\,A^T\\,A\\,x = A\\,x \cdot A\\,x \\\\
      &= \lVert A\\,x \rVert\_2^2
  \end{aligned}
\\]

对于正交矩阵 \\( A \in \mathbb{R}^{n \times n} \\)，
我们有 \\( \lVert A\\,x \rVert\_2 = \lVert x \rVert\_2 \\)

\\[
  \begin{aligned}
    \lVert A\\,x \rVert\_2^2 &= (A\\,x) \cdot (A\\,x) = x^T\\,A^T\\,A\\,x \\\\
      &= x^T\\,\mathbf{I}\\,x = x^T\\,x \\\\
      &= \lVert x \rVert\_2^2
  \end{aligned}
\\]
...
- 酉矩阵：对于 \\( A \in \mathbb{C}^{n \times n} \\)，如果 \\( A^\dagger\\,A = \mathbf{I} \\)，那么 \\( A \\) 就是酉矩阵
...
定义 \\( A \in \mathbb{R}^{m \times n} \\) 的 \\( L \\)-\\( \infty \\) 模为：\\( \max\_{1 \le i \le m}{\sum\_{j = 1}^n{|a\_{i\\,j}|}} \\)，
即各个行向量元素的绝对值的和的最大值
...
\\[
  \lVert{}A\rVert{}\_F = \sqrt{\sum\_{i = 1}^m{\sum\_{j = 1}^n{a\_{i\\,j}^2}}}
\\]
