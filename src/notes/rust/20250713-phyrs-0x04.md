...
设哈雷彗星距离太阳最近的位置为 $\left(-8.768 \times 10^{10}\,\mathrm{m}, 0\right)$，
此时速度为 $\left(0, 54569\,\mathrm{m/s}\right)$[^1]

太阳质量约为 $1.9885 \times 10^{30}\,\mathrm{kg}$，
...
fn run() -> Option<()> {
    ...
    let state = Particle::<Double, 2>::without_charge(
        Double::of(2.2e14),
        VectorC::of(&[-Double::of(8.768e10), Double::zero()])?,
        VectorC::of(&[Double::zero(), Double::of(5.4569e4)])?, // 顺时针
        Double::zero(),
    );
    // 合力
    let total_acc = Arc::new(|s: Particle<Double, 2>| -> VectorC<Double, 2> {
        let gravity_constant = Double::of(6.6743e-11);
        let sun_mass = Double::of(1.9885e30);
        // Fg = - (G M m / |r|^2) r/|r|
        -normalize(&s.position) * gravity_constant * sun_mass
            / euclidean_norm(&s.position).power(Double::of(2.0))
    });
    // 模拟计算
    let one_step = Particle::rk4_transition(total_acc, Double::of(24.0 * 60.0 * 60.0));
    let states = state
        .simulate(one_step)
        .take_while(|s| s.time <= Double::of(76.0 * 365.25 * 24.0 * 60.0 * 60.0))
        .map(|s| {
            (
                s.time.raw(),
                s.position[(1, 1)].raw(),
    ...
    let draw_area = BitMapBackend::gif("result/draw_halley.gif", (720, 320), 1)
        .ok()?
        .into_drawing_area();
    for s in states.iter().step_by(32) {
...
其中通过状态模拟得到的哈雷彗星的周期约是 $75.7$ 年，与实际结果还是很接近的
...
fn run() -> Option<()> {
    ...
    let total_acc = Arc::new(|s: Particle<Double, 3>| -> VectorC<Double, 3> {
        let m = s.mass.clone();
        lorentz_force(VectorC::default(), basis_vector(3) * Double::of(3.2e-8), s) / m
    });

    let one_step = Particle::rk4_transition(total_acc, Double::of(0.01));
    let states = state
        .simulate(one_step)
    ...
    let draw_area = BitMapBackend::gif("result/draw_lorentz.gif", (512, 600), 1)
        .ok()?
        .into_drawing_area();
    let points = states.step_by(5).map(|s| {
        (
            s.time,
            s.position[(1, 1)].raw(),
            s.position[(2, 1)].raw(),
...
但是上述公式得到的是加速度，为了统一形式，
可以将 `default_transition` 和 `rk4_transition` 的参数由力改成加速度[^2]：

```rust
impl<R: RealFloat + 'static, const E: usize> Particle<R, E> {
    pub fn default_transition(
        acceleration: Arc<dyn Fn(Self) -> VectorC<R, E>>,
        dt: R,
    ) -> Arc<dyn Fn(Self) -> Self> {
        Arc::new(move |p: Self| -> Self {
...
pub fn relativity_acceleration<R: RealFloat, const E: usize>(
    p: Particle<R, E>,
    speed_of_light: R,
    force: Arc<dyn Fn(Particle<R, E>) -> VectorC<R, E>>,
) -> VectorC<R, E> {
...
fn run() -> Option<()> {
    ...
    let const_force = Arc::new(|_p: Particle<Double, 1>| -> VectorC<Double, 1> {
        basis_vector(1) * Double::of(10.0)
    });
    let const_force_copy = const_force.clone();
    let newton_acc = Arc::new(move |p: Particle<Double, 1>| -> VectorC<Double, 1> {
        const_force(p.clone()) / p.mass
    });
    let newton_step = Particle::rk4_transition(newton_acc, one_day.clone());
    let relativity_acc = Arc::new(move |p: Particle<Double, 1>| -> VectorC<Double, 1> {
...
fn run() -> Option<()> {
    ...
    let force = Arc::new(|p: Particle<Double, 3>| {
        lorentz_force(VectorC::default(), basis_vector(3) * Double::one(), p)
    });
    let force_copy = force.clone();

    let newton_acc = Arc::new(move |s: Particle<Double, 3>| -> VectorC<Double, 3> {
        let m = s.mass.clone();
        force(s) / m
    });
    ...
    let relativity_acc = Arc::new(move |s: Particle<Double, 3>| -> VectorC<Double, 3> {
