...
```rust
pub fn derivative_vec<R: RealFloat + 'static, const E: usize>(
    f: Arc<dyn Fn(R) -> VectorC<R, E>>,
    dx: R,
) -> Arc<dyn Fn(R) -> VectorC<R, E>> {
    Arc::new(move |x: R| -> VectorC<R, E> {
...
fn run() -> Option<()> {
    let force = Arc::new(|t: Double| -> Double {
        if (t / Double::of(10.0))
            .truncate::<Int>()
            .modulus(Int::of(2))
            .is_zero()
    ...
    // 状态变更
    let one_step = Arc::new(|s: State, dt: Double| {
