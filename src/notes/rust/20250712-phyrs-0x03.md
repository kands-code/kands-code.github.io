...
impl<R: RealFloat + 'static, const E: usize> Particle<R, E> {
    pub fn default_transition(
        total_force: Arc<dyn Fn(Self) -> VectorC<R, E>>,
        dt: R,
    ) -> Arc<dyn Fn(Self) -> Self> {
        Arc::new(move |p: Self| -> Self {
            let acceleration = total_force(p.clone()) / p.mass.clone();
            let velocity = p.velocity.clone() + acceleration.clone() * dt.clone();
    ...
    pub fn simulate(self, transition: Arc<dyn Fn(Self) -> Self>) -> Iterate<Self> {
...
若劲度系数 $k = 0.8\,\mathrm{kg/s^2}$，初始位置为 $\left(0, 0, 10\,\mathrm{cm}\right)$，初速度为 $\vec{v} = \vec{0}$，则：
...
fn run() -> Option<()> {
    // 初始状态
    let state = Particle::<Double, 3> {
        mass: Double::of(2.7e-3),
        position: VectorC::of(&[0.0, 0.0, 0.1].map(Double::of))?,
        velocity: VectorC::default(),
        time: Double::zero(),
    };
    // 合力
    let total_force = Arc::new(|s: Particle<Double, 3>| -> VectorC<Double, 3> {
        let g = VectorC::of(&[
            Double::zero(),
            Double::zero(),
            -Double::PI.power(Double::of(2.0)),
        ])
        .expect("Error[total_force]: 构建重力加速度失败.");
        g * s.mass.clone() // 重力
            - s.position.clone() * Double::of(0.8) // 弹力
            - s.velocity // 空气阻力
    ...
    // 模拟计算
    let one_step = Particle::default_transition(total_force, Double::of(0.015625));
    // 计算 0 到 3 秒内的运动轨迹
    let states = state
        .simulate(one_step)
    ...
    chart
        .draw_series(LineSeries::new(
            states.map(|s| (s.time.raw(), s.position[(3, 1)].raw())),
            &BLUE_400,
        ))
        .ok()?;
...
与欧拉法的区别在于，通常的欧拉法使用的是上一个状态的速度来计算下一个状态的位移，
...
欧拉-克罗默方法比欧拉法更遵守能量守恒，精度和稳定性也比欧拉法高
...
pub fn rk4_transition(
    ...
    Arc::new(move |p1: Self| -> Self {
        ...
        // k1
        let k1_dv_dt = total_force(p1.clone()) / p1.mass.clone();
        ...
        let k2_dv_dt = total_force(p2.clone()) / p2.mass.clone();
        ...
        let k3_dv_dt = total_force(p3.clone()) / p3.mass.clone();
        ...
        let k4_dv_dt = total_force(p4.clone()) / p4.mass.clone();
...
fn run() -> Option<()> {
    // 初始状态
    let state = Particle::<Double, 1> {
        mass: Double::one(),
        position: VectorC::of(&[Double::of(2.0)])?,
        velocity: VectorC::default(),
        time: Double::zero(),
    };
    // 合力
    let total_force = |mu: f64| -> Arc<dyn Fn(Particle<Double, 1>) -> VectorC<Double, 1>> {
        Arc::new(move |s: Particle<Double, 1>| -> VectorC<Double, 1> {
            basis_vector(1)
                * Double::of(mu)
    ...
    let states0 = state
        ...
        .take_while(|s| s.time <= Double::of(100.0));
    let states2 = state
        .clone()
        .simulate(Particle::rk4_transition(
            total_force(2.0),
