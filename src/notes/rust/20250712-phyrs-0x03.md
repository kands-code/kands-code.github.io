...
\\[
\begin{gathered}
  \vec{a}\_n = \dfrac{\vec{F}\_n\left(t\_n, \vec{r}\_n, \vec{v}\_n\right)}{m},
    \quad\vec{v}\_{n + 1} = \vec{v}\_n + \vec{a}\_n \Delta t \\\\
  \vec{r}\_{n + 1} = \vec{r}\_n + \vec{v}\_{n + 1} \Delta t,
    \quad t\_{n + 1} = t\_n + \Delta t
 \end{gathered}
\\]
...
\\[
  \left(m, \vec{r}\_0, \vec{v}\_0, t\_0\right) \to \left(m, \vec{r}\_1, \vec{v}\_1, t\_1\right)
    \to\cdots\to\left(m, \vec{r}\_n, \vec{v}\_n, t\_n\right)
\\]
...
而是 \\( z\_0 = m g / k \approx 0.033\\,\mathrm{m} \\)
...
\\[
  \begin{gathered}
    F\_\textnormal{total} = F\_k + F\_\textnormal{damp} \\\\\\\\
    F\_k = -x,\quad F\_\textnormal{damp} = \mu (1 - x^2) v
  \end{gathered}
\\]
