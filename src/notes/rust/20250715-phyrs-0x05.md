...
| :--------------------------------------: | :------: | :--------------------------------------------------------------------: | :---------------------------------------------------------------------: |
| \\( \lVert\vec{r}\_{B A}\rVert > r_e \\) |   拉伸   | \\( k \left(\lVert\vec{r\_{B A}}\rVert - r_e\right) \hat{r}\_{B A} \\) | \\( -k \left(\lVert\vec{r\_{B A}}\rVert - r_e\right) \hat{r}\_{B A} \\) |
| \\( \lVert\vec{r}\_{B A}\rVert = r_e \\) |   平衡   |                               \\( 0 \\)                                |                                \\( 0 \\)                                |
| \\( \lVert\vec{r}\_{B A}\rVert < r_e \\) |   压缩   | \\( k \left(\lVert\vec{r\_{B A}}\rVert - r_e\right) \hat{r}\_{B A} \\) | \\( -k \left(\lVert\vec{r\_{B A}}\rVert - r_e\right) \hat{r}\_{B A} \\) |
...
\\[
  \begin{gathered}
    ...
    \dfrac{\mathrm{d}}{\mathrm{d} t} \left[\vec{v}\_n(t)\right] = \dfrac{1}{m\_n}\left(
      \vec{F}\_{\textnormal{ex}\left[n\right]}\left(t, \vec{r}\_n(t), \vec{v\}\_n(t)\right)
        + \sum\_m \vec{F}\_{n m}\left(\vec{r}\_n(t) - \vec{r}\_m(t), \vec{v}\_n(t) - \vec{v}\_m(t)\right)\right)
  \end{gathered}
\\]
