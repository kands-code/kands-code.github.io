...
```rust
pub fn linear_spring<'a, R: RealFloat + 'a>(
    k: R,
    original_length: R,
) -> Rc<dyn Fn(Particle<R, 1>, Particle<R, 1>) -> VectorC<R, 1> + 'a> {
    Rc::new(
        move |center: Particle<R, 1>, p: Particle<R, 1>| -> VectorC<R, 1> {
            let dr = p.position - center.position;
...
```rust
pub fn fixed_linear_spring<'a, R: RealFloat + 'a>(
    k: R,
    original_length: R,
    fixed_postion: VectorC<R, 1>,
) -> Rc<dyn Fn(Particle<R, 1>) -> VectorC<R, 1> + 'a> {
    Rc::new(move |p: Particle<R, 1>| -> VectorC<R, 1> {
...
impl<R: RealFloat + 'static, const E: usize> ParticleSystem<R, E> {
    pub fn force_to_acceleration(
        ...
        let f2a_capture = force_to_acceleration.clone();
        match force {
...
fn run() -> Option<()> {
    ...
    let one_step = ParticleSystem::system_transition(
        ...
        f2a,
        Double::of(0.01),
        Rc::new(Particle::rk4_transition),
    );
    let states = system
        .simulate(one_step)
