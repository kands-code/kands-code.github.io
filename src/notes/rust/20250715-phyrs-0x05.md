...
pub fn central_force<'a, R: RealFloat + 'a, const E: usize>(
    ...
    Rc::new(
        move |center: Particle<R, E>, p: Particle<R, E>| -> VectorC<R, E> {
            ...
            let distance = euclidean_norm(&dr);
            let direction_vector = normalize(&dr);
            direction_vector * relation(distance)
        },
    )
}
```
...
pub fn universal_gravity<'a, R: RealFloat + 'a, const E: usize>(
    ...
    let relation = Rc::new(move |distance: R| -> R {
        let gravity_constant = real_to_frac::<Double, R>(Double::of(6.6743e-11));
        -gravity_constant * m1.clone() * m2.clone() / distance.power(R::one() + R::one())
    });
...
impl<R: RealFloat + 'static, const E: usize> ParticleSystem<R, E> {
    ...
    pub fn system_adaptive_transition(
        forces: Vec<SystemForce<R, E>>,
        f2a: Rc<
            dyn Fn(
                Rc<dyn Fn(Particle<R, E>) -> VectorC<R, E>>,
        ...
        >,
        rtol: R,
        particle_adapter_transition: Rc<
            dyn Fn(
                Rc<dyn Fn(Particle<R, E>) -> VectorC<R, E>>,
                R,
            ) -> Rc<dyn Fn((Particle<R, E>, R)) -> (Particle<R, E>, R)>,
        >,
    ) -> Rc<dyn Fn((Self, R)) -> (Self, R)> {
        let f2a_capture = f2a.clone();
        Rc::new(
            move |(system, dt): (ParticleSystem<R, E>, R)| -> (ParticleSystem<R, E>, R) {
                let states = system
                    .particles
                    .iter()
                    .enumerate()
                    .map(|(idx, p)| {
                        ...
                        particle_adapter_transition(acceleration, rtol.clone())((
                            p.clone(),
                            dt.clone(),
                        ))
                    })
                ...
                (
                    ParticleSystem {
                        particles: states.iter().map(|(p, _)| p.clone()).collect::<Vec<_>>(),
                        time: system.time + dt.clone(),
                    },
                    (states
                        .iter()
                        .map(|(_, dt)| dt.clone().power(R::one() + R::one()))
                        .fold(R::zero(), |acc, e| acc + e)
                        / from_integral(Word::of(states.len() as u32)))
                    .square_root(),
                )
            },
        )
    ...
    pub fn simulate_adaptive<'a>(
        self,
        transition: Rc<dyn Fn((Self, R)) -> (Self, R) + 'a>,
        init_dt: R,
    ) -> Iterate<'a, (Self, R)> {
        Iterate::of(transition, (self, init_dt))
    }
}
```
...
fn run() -> Option<()> {
    ...
    let states = system
        .simulate_adaptive(one_step, Double::of(0.01))
        .map(|(s, _)| s)
        .take_while(|s| s.time < Double::of(64.0))
        .map(|s| {
            (
                s.time,
                s.particles
