...
代码必须使用的依赖是 **rmatrix\_ks** 库，用于向量和矩阵运算以及描述数字
...
\\[
  \overline{v}\_{\left[t\_0,\\,t\_1\right]} = \dfrac{x(t\_1) - x(t\_0)}{t\_1 - t\_0}
\\]

如果时间间隔无限趋近于 0，此时平均速度就可以表示物体运动的瞬时速度，即：

\\[
  v\_{t\_0} = \lim\_{t\_1 \to t\_0} \dfrac{x(t\_1) - x(t\_0)}{t\_1 - t\_0}
\\]

或者使用 \\( \Delta t \\) 表示间隔，可以得到：

\\[
  v\_{t} = \lim\_{\Delta t \to 0} \dfrac{x(t + \Delta t / 2) - x(t - \Delta t / 2)}{\Delta t}
\\]
...
其中 `RealFloat` 来自 **rmatrix\_ks** 库，使用 `Rc` 是为了避免闭包的所有权问题，
...
\\[
  v\_\textnormal{Car} = \dfrac{\mathrm{d} x\_\textnormal{Car}}{\mathrm{d} t} = - \sin{t}
\\]
...
\\[
  a = \dfrac{\mathrm{d} v}{\mathrm{d} t}
    = \lim\_{\Delta t \to 0} \dfrac{v(t + \Delta t / 2) - v(t - \Delta t / 2)}{\Delta t}
\\]
...
\\[
  \int\_a^b f(x) \mathrm{d}x
    = \lim\_{N \to \infty} \sum\_{n = 0}^{N / 2 - 1}
...
\\[
  F(x) = F\_\textnormal{init} + \int\_0^x f(t) \mathrm{d} t
\\]
...
`Iterate` 是一个迭代器，将返回 \\( \left(f(x\_0), f(f(x\_0)), f(f(f(x\_0))), \cdots \right) \\)，
其具体定义如下：
