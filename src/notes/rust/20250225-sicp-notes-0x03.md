...
处于简单考虑，这里没有处理引用和所有权问题，如果想要优化，可以尝试使用引用
...
$$
\dfrac{\dfrac{2 x (x^3 - 8 x^2 + 3)}{(x^2 + 3) (3 x^2)}}{(x^3 - 8 x^2 + 3)^2}
$$

可以看到，还是有很多优化空间的，我们再手动优化一下可以得到：

$$
\dfrac{2}{3 x (x^2 + 3) (x^3 - 8 x^2 + 3)}
$$
...
在早期的 **Maxima**，符号微分的实现就是用类似的 LISP 代码来实现的，
