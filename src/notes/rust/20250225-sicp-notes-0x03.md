...
所以一个线段可以表示为 `$s = \lbrace (x_a, y_a), (x_b, y_b) \rbrace$`，即：
...
fn is_zero(x: &f64) -> bool { x.abs() <= core::f64::EPSILON.sqrt() }
...
impl DiffElement {
    ...
    pub fn make_div(e1: DiffElement, e2: DiffElement) -> DiffElement {
        match (&e1, &e2) {
            (DiffElement::Constant(a), DiffElement::Constant(b)) if !is_zero(b) => DiffElement::Constant(a / b),
            (DiffElement::Constant(a), _) if is_zero(a) => DiffElement::Constant(0.0),
            (_, DiffElement::Constant(a)) if is_zero(&(a - 1.0)) => e1,
    ...
    pub fn diff(&self) -> DiffElement {
        match self {
            ...
            DiffElement::Mul(e1, e2) => DiffElement::make_add(
                DiffElement::make_mul(e1.diff(), *e2.clone()),
                DiffElement::make_mul(*e1.clone(), e2.diff()),
            ),
            DiffElement::Div(e1, e2) => DiffElement::make_div(
                DiffElement::make_add(
                    ...
                    DiffElement::make_mul(
                        ...
                        DiffElement::make_mul(*e1.clone(), e2.clone().diff()),
                    ),
                ),
                DiffElement::make_pow(*e2.clone(), 2.0),
            ),
...
|   |   |   |   |   Mul(Constant(-8.0), Mul(Constant(2.0), Variable))
|   |   |   |   )
|   |   |   )
|   |   )
|   ),
...
|   |   Add(
|   |   |   Pow(Variable, 3.0),
|   |   |   Add(Mul(Constant(-8.0), Pow(Variable, 2.0)), Constant(3.0))
|   |   ),
|   |   2.0
|   )
)
...
```math
\dfrac{2 x (x^3 + (-8 x^2 + 3)) + (- (x^2 + 3) (3 x^2 + (- 8 (2 x))))}{(x^3 + (-8 x^2) + 3)^2}
```

可以看到，还是有很多优化空间的，我们再手动优化一下可以得到：

```math
\dfrac{2 x}{x^3 -8 x^2 + 3}
    - \dfrac{(3 x^2 - 16 x)(x^2 + 3)}{(x^3 -8 x^2 + 3)^2}
```
